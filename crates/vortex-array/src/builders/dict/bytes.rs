...
impl<Code: UnsignedPType> BytesDictBuilder<Code> {
    ...
    fn encode_value(&mut self, lookup: &mut HashTable<Code>, val: &[u8]) -> Option<Code> {
        match lookup.entry(
            ...
            HashTableEntry::Vacant(vacant) => {
                // M28 (openobserve vendored patch): the FIRST entry of a fresh
                // dictionary is always admitted, even when it alone violates
                // max_len/max_bytes. Refusing it makes zero progress: callers
                // (vortex-layout `DictStreamState::encode`) close the run and
                // retry the same leading value on another fresh dictionary,
                // looping forever while allocating a codes+values pair per
                // iteration (prod compactor live-heap runaway, ~50MB/s per
                // stuck write). An oversized first entry instead closes the
                // run on the NEXT value, so every previously-completing input
                // encodes byte-identically.
                let next_code = self.views.len();
                if next_code > 0 && next_code >= self.max_dict_len {
                    return None;
                }
                ...
                if next_code > 0 && self.dict_bytes() + additional_bytes > self.max_dict_bytes {
                    return None;
                }
    ...
    fn encode_null(&mut self) -> Option<Code> {
        ...
        // M28 (openobserve vendored patch): see `encode_value` — the first
        // entry of a fresh dictionary is always admitted so encoding can
        // make progress under degenerate constraints.
...
mod test {
    ...
    /// M28 (openobserve vendored patch): a value larger than
    /// `max_bytes` must still be admitted as the FIRST entry of a fresh
    /// dictionary — refusing it lets `vortex-layout`'s dict writer loop
    /// forever re-trying the same leading value on fresh dictionaries.
    /// The oversized entry then closes the run on the next value.
    #[test]
    fn oversized_first_value_is_admitted() -> VortexResult<()> {
        use crate::builders::dict::DictConstraints;
        use crate::builders::dict::dict_encoder;

        let big = "B".repeat(100);
        let arr = VarBinViewArray::from_iter_str(vec![big.as_str(), "b", "b", "c"]).into_array();
        let constraints = DictConstraints {
            max_bytes: 64, // smaller than the leading value
            max_len: usize::MAX,
        };
        let mut ctx = SESSION.create_execution_ctx();
        let mut encoder = dict_encoder(&arr, &constraints);

        // The oversized leading value is admitted (progress), and the run
        // closes immediately after it: exactly one row encodes.
        let codes = encoder.encode(&arr, &mut ctx)?;
        assert_eq!(codes.len(), 1, "the oversized first entry must encode");
        let values = encoder.reset();
        assert_eq!(values.len(), 1, "run closes with the single oversized entry");

        // The remainder then encodes on a fresh dictionary as usual.
        let rest = arr.slice(1..4)?;
        let codes = encoder.encode(&rest, &mut ctx)?;
    ...
    /// M28: degenerate `max_len = 0` must also admit the first entry rather
    /// than refuse every value forever.
    #[test]
    fn zero_max_len_still_progresses() -> VortexResult<()> {
        use crate::builders::dict::DictConstraints;
        use crate::builders::dict::dict_encoder;

        let arr = VarBinViewArray::from_iter_str(vec!["x", "y"]).into_array();
        let constraints = DictConstraints {
            max_bytes: usize::MAX,
            max_len: 0,
        };
