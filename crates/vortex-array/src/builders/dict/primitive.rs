    ...
    Code: UnsignedPType,
{
    ...
    fn encode_value(&mut self, v: T) -> Option<Code> {
        match self.lookup.entry(NativeValue(v)) {
            ...
            Entry::Vacant(vac) => {
                // M28 (openobserve vendored patch): always admit the FIRST
                // entry of a fresh dictionary (see bytes.rs `encode_value`) so
                // encoding makes progress even when `max_dict_len` computes to
                // 0 under degenerate constraints (max_bytes < byte_width).
    ...
    fn encode_null(&mut self) -> Option<Code> {
        ...
        // M28 (openobserve vendored patch): see `encode_value` above.
        if !self.values.is_empty() && self.values.len() >= self.max_dict_len {
            return None;
        }
...
mod test {
    ...
    /// M28 (openobserve vendored patch): degenerate constraints
    /// (`max_bytes` below the element width => computed `max_dict_len == 0`)
    /// must still admit the first entry so encoding makes progress.
    #[test]
    fn degenerate_constraints_still_progress() {
        use crate::builders::dict::DictConstraints;
        use crate::builders::dict::dict_encoder;

        let arr = PrimitiveArray::from_iter([1i64, 2, 3]).into_array();
        let constraints = DictConstraints {
            max_bytes: 4, // < 8-byte element width => max_dict_len == 0
            max_len: usize::MAX,
        };
        let mut ctx = SESSION.create_execution_ctx();
        let mut encoder = dict_encoder(&arr, &constraints);
        let codes = encoder.encode(&arr, &mut ctx).unwrap();
        assert_eq!(codes.len(), 1, "first entry admitted");
