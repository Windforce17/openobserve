...
//!
//! The build/rebuild paths accumulate `composite term -> doc ids` in a
//! `BTreeMap` until `finish` because the on-disk index needs terms in global
...
/// Estimated resident overhead of one map entry beyond its key bytes and
/// 4-byte postings: BTreeMap node share + two Vec headers + allocation
/// rounding. Deliberately rough — it only decides WHEN to spill.
pub(crate) const PER_TERM_OVERHEAD: usize = 88;
...
impl TermSpill {
    ...
    /// Drain `terms` (ascending key order by construction) into a new sorted
    /// run file. The map is left empty.
    pub(crate) fn write_run(&mut self, terms: &mut BTreeMap<Vec<u8>, Vec<u32>>) -> Result<()> {
        ...
        for (key, ids) in map {
            ...
            writer
                .write_all(&(ids.len() as u32).to_le_bytes())
                .map_err(io)?;
            for id in ids {
...
impl RunReader {
    ...
    pub(crate) fn take_current(&mut self) -> Option<(Vec<u8>, Vec<u32>)> {
        self.current.take()
    }
    ...
    pub(crate) fn advance(&mut self) -> Result<()> {
        ...
        let mut docs_bytes = vec![0u8; doc_count * 4];
        self.reader.read_exact(&mut docs_bytes).map_err(io)?;
        let ids: Vec<u32> = docs_bytes
            .chunks_exact(4)
...
/// K-way merge of spill runs plus the final resident map, in ascending key
...
/// the resident map last) — the caller-guaranteed doc-range ordering is
...
pub(crate) fn merge_spilled_terms(
    ...
    resident: BTreeMap<Vec<u8>, Vec<u32>>,
    mut emit: impl FnMut(Vec<u8>, Vec<u32>) -> Result<()>,
) -> Result<()> {
    ...
    loop {
        ...
        for run in &runs {
            if let Some((key, _)) = run.current() {
                if min_key.is_none() || key.as_slice() < min_key.unwrap() {
                    min_key = Some(key.as_slice());
                }
            }
        }
        if let Some((key, _)) = resident.peek() {
            if min_key.is_none() || key.as_slice() < min_key.unwrap() {
                min_key = Some(key.as_slice());
            }
        }
        ...
        for run in &mut runs {
            if run.current().is_some_and(|(key, _)| key == &min_key) {
                let (_, mut docs) = run.take_current().expect("checked above");
        ...
        if resident.peek().is_some_and(|(key, _)| key == &min_key) {
            let (_, mut docs) = resident.next().expect("peeked above");
            check_run_boundary(&min_key, &ids, &docs)?;
            if ids.is_empty() {
                ids = std::mem::take(&mut docs);
            } else {
...
mod tests {
    ...
    fn spill_roundtrip_and_merge() {
        ...
        // run 0: docs 0..10, run 1: docs 10..20, resident: docs 20..30
        let mut run0 = BTreeMap::new();
        run0.insert(b"alpha".to_vec(), vec![0, 3]);
        run0.insert(b"beta".to_vec(), vec![1]);
        spill.write_run(&mut run0).unwrap();
        assert!(run0.is_empty());

        let mut run1 = BTreeMap::new();
        run1.insert(b"alpha".to_vec(), vec![10, 12]);
        run1.insert(b"gamma".to_vec(), vec![11]);
        spill.write_run(&mut run1).unwrap();

        let mut resident = BTreeMap::new();
        resident.insert(b"beta".to_vec(), vec![25]);
        resident.insert(b"delta".to_vec(), vec![21]);

        assert_eq!(spill.run_count(), 2);
        let (runs, _dir) = spill.into_run_readers().unwrap();
        let mut merged: Vec<(Vec<u8>, Vec<u32>)> = Vec::new();
        merge_spilled_terms(runs, resident, |key, ids| {
            merged.push((key, ids));
            Ok(())
        })
        .unwrap();
        ...
        assert_eq!(
            merged,
            vec![
                (b"alpha".to_vec(), vec![0, 3, 10, 12]),
                (b"beta".to_vec(), vec![1, 25]),
                (b"delta".to_vec(), vec![21]),
                (b"gamma".to_vec(), vec![11]),
            ]
        );
    }
    ...
    fn out_of_order_runs_are_rejected() {
        ...
        let mut spill = TermSpill::new(base.path()).unwrap();
        let mut run0 = BTreeMap::new();
        run0.insert(b"key".to_vec(), vec![5]);
        spill.write_run(&mut run0).unwrap();
        let mut resident = BTreeMap::new();
        resident.insert(b"key".to_vec(), vec![5]); // duplicate doc across runs
        let (runs, _dir) = spill.into_run_readers().unwrap();
        let result = merge_spilled_terms(runs, resident, |_, _| Ok(()));
