...
impl BlockSegmentPostings {
    ...
    /// Number of postings with doc_id strictly less than `target`.
    /// Jumps via the skip list (no decode of skipped blocks) + decodes one block.
    /// Designed to be called with non-decreasing targets (galloping).
